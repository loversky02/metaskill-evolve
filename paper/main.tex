...
\textbf{Skill-evolution overfits its training target; recursion regularizes.} Our
sharpest live result comes from a held-out \emph{target-transfer} test: evolve the
counting skill on one letter (`r'), then measure on a \emph{different} letter (`e',
$n{=}15$). The seed skill scores $0.73$. Single-level evolution \emph{degrades} it to
$0.33$ on transfer---and the evolved skill shows exactly why: it hardcodes the
training target, instructing the model to ``circle every instance of `r' or `R'\,''
regardless of the question. The fast loop overfit the training distribution. The
two-level system partially corrects this ($0.60$; a $+0.27$ meta-gain over
single-level), producing a more general procedure---the slow loop's value surfacing
as \emph{robustness}, not raw accuracy. Neither beats the seed here (the whole
evolution still net-hurts on transfer, and $n{=}15$ leaves a $\pm0.11$ band), but the
direction is the honest takeaway: unconstrained self-improvement can overfit its own
edits, and the recursive layer acts as a mild regularizer against it. This is the
live counterpart to the paper's frozen-backbone claim---and a caution the hype omits.
