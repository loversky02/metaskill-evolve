...
that verify the mechanism and the harness, not benchmark results.} Live
confirmation on Gemma-4 E2B/E4B (local MLX) and GPT-5.5/Claude (gateway) is
ongoing; see Table~\ref{tab:law-live} (\textsc{TODO}).
...
\begin{table}[h]\centering
\caption{\textsc{TODO} (live): meta-gain vs.\ real backbone strength.}
\label{tab:law-live}
\begin{tabular}{lccc}
\toprule
Backbone & Single-level & Two-level & Meta-gain\\
\midrule
Gemma-4 E2B (MLX) & \textsc{todo} & \textsc{todo} & \textsc{todo}\\
Gemma-4 E4B (MLX) & \textsc{todo} & \textsc{todo} & \textsc{todo}\\
Claude / GPT-5.5  & \textsc{todo} & \textsc{todo} & \textsc{todo}\\
\bottomrule
\end{tabular}
\end{table}
