...
\begin{table}[h]\centering
\caption{Live probe: both failure modes. On easy word problems the base policy is
already at ceiling ($U_0{=}1.0$); on knowledge-hard SealQA (no tools) it is at the
floor ($U_0{=}0.0$). Either way the recursion has no headroom, so the meta-gain is
$0$ --- the law needs a procedure-bottlenecked task in between, not stronger claims
from this data.}
\label{tab:law-live}
\begin{tabular}{llcccc}
\toprule
Task & Backbone & $U_0$ & Single & Two-level & Meta-gain\\
\midrule
Word problems & Gemma-2-2B (MLX)   & 1.00 & 1.00 & 1.00 & $+0.00$ (ceiling)\\
Word problems & Claude Haiku 4.5   & 1.00 & 1.00 & 1.00 & $+0.00$ (ceiling)\\
SealQA Seal-Hard & Claude Haiku 4.5 (no tools) & 0.00 & \multicolumn{3}{c}{floor -- not run}\\
\bottomrule
\end{tabular}
\end{table}
